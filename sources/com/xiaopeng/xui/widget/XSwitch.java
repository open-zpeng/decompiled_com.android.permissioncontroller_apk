...
public class XSwitch extends XCompoundButton implements ValueAnimator.AnimatorUpdateListener {
    ...
    private void resetAnimValue() {
        if (isChecked()) {
            this.mCurrentAnimValue = ANIMATOR_DURATION;
        } else {
    ...
    private void updateVerticalFraction() {
        ...
        if (i <= 200) {
            ...
            this.mVerticalLightColor = changeColorAlpha((this.mBlurAlphaDst * i) / 200.0f, this.mVerticalColor);
        } else if (i <= 500) {
            ...
            this.mVerticalLightColor = changeColorAlpha(f5 - ((((i - 300) - 200) * f5) / 200.0f), this.mVerticalColor);
        }
    }
